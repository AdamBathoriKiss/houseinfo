...
export default function Announcements({ title, content, author, authorId, date, buildingId }: CurrentNews) {
	const { user } = useAuth();
	...
	useEffect(() => {
		...
		if (author) {
			...
			setValue("authorId", author.id ? author.id : user.userId);
		}
		if (!author) {
	...
	}, [title, content, author, authorId, date, setValue]);
			...
			<form onSubmit={handleSubmit(onSubmit)} className="p-3">
				<div className="grid grid-cols-1 gap-4">
					<div className="flex flex-col col-start-1 col-end-12 gap-2">
						<label className="font-semibold text-gray-100 text-sm">Cím *</label>
						<input
							{...register("title")}
							type="text"
					...
					<div className="flex flex-row col-start-1 col-end-12 gap-2">
						<div className="w-1/2">
							<label className="font-semibold text-gray-100 text-sm">Létrehozta *</label>
							<input
								{...register("author")}
								type="text"
						...
						{errors.author && <span className="text-red-500 text-xs">{errors.author.message}</span>}
						<div className="w-1/2">
							<label className="font-semibold text-gray-100 text-sm">Létrehozva *</label>
							<input
								{...register("date")}
								type="date"
								placeholder="pl. Fő utca 123."
					...
					<div className="flex flex-col gap-2 col-start-1 col-end-12">
						<label className="font-semibold text-gray-100 text-sm">Hír szövege *</label>
						<InputTextarea
							{...register("content")}
							rows={10}
							cols={95}
